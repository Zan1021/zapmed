...
- [x] 5.5 Standalone Forge site + separate Postgres DB provisioned; deploy; health check green. **DONE — LIVE at https://spar.zapmed.africa** (own Forge site on zapdev 143.244.215.131, own Postgres DB `spar_standalone`; NFR-4/NFR-5 isolation confirmed). Full Close-the-Loop + Health Coach build deployed 2026-09-22 (composer install, 6 migrations run, `npm run build`, caches warmed). Health check green: `/`, `/login`, `/spar/dashboard` → 200; SSL valid to 2026-12-10; browser-verified admin login + Exceptions screen with B6 Nudge/Snooze/Message action buttons rendering. WhatsApp on `driver=log` (no real sends during client testing). ⚠ For REAL patient PHI on staging, Phase-10 go-live blockers (MFA, breach process, pen-test, compliance sign-off) still apply. — est: M
