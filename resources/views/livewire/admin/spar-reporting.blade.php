<div>
    <x-slot name="header">SPAR Reporting & Analytics</x-slot>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-black {{ $this->adherenceStats['adherence_rate'] >= 80 ? 'text-green-600' : ($this->adherenceStats['adherence_rate'] >= 60 ? 'text-amber-600' : 'text-red-600') }}">
                    {{ $this->adherenceStats['adherence_rate'] }}<span class="text-xl">%</span>
                </p>
                <div class="mt-3 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-700 {{ $this->adherenceStats['adherence_rate'] >= 80 ? 'bg-green-500' : ($this->adherenceStats['adherence_rate'] >= 60 ? 'bg-amber-500' : 'bg-red-500') }}" style="width: {{ $this->adherenceStats['adherence_rate'] }}%"></div>
                </div>
                <p class="text-xs text-gray-500 mt-2">{{ $this->adherenceStats['on_time'] }} of {{ $this->adherenceStats['total_due'] }} on time</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-black text-purple-600">{{ $this->renewalStats['renewal_rate'] }}<span class="text-xl">%</span></p>
                <div class="mt-3 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full bg-purple-500 rounded-full transition-all duration-700" style="width: {{ $this->renewalStats['renewal_rate'] }}%"></div>
                </div>
                <p class="text-xs text-gray-500 mt-2">{{ $this->renewalStats['renewed'] }} renewed, {{ $this->renewalStats['renewal_due'] }} pending</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-black text-blue-600">{{ $this->patientStats['opt_in_rate'] }}<span class="text-xl">%</span></p>
                <div class="mt-3 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div class="h-full bg-blue-500 rounded-full transition-all duration-700" style="width: {{ $this->patientStats['opt_in_rate'] }}%"></div>
                </div>
                <p class="text-xs text-gray-500 mt-2">{{ $this->patientStats['consented'] }} of {{ $this->patientStats['total'] }} patients</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-4xl font-black text-emerald-600">{{ $this->renewalStats['zapmed_conversion'] }}<span class="text-xl">%</span></p>
                <div class="flex items-center gap-4 mt-3">
                    <div class="flex items-center gap-1">
                        <div class="w-2 h-2 rounded-full bg-emerald-500"></div>
                        <span class="text-xs text-gray-500">ZapMed: {{ $this->renewalStats['via_zapmed'] }}</span>
                    </div>
                    <div class="flex items-center gap-1">
                        <div class="w-2 h-2 rounded-full bg-gray-300"></div>
                        <span class="text-xs text-gray-500">GP: {{ $this->renewalStats['via_primary_doctor'] }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="flex-1 flex flex-col items-center gap-2">
                        <div class="w-full flex flex-col items-center justify-end h-40 relative">
                            ...
                            <div class="w-full bg-gray-100 rounded-t-lg absolute bottom-0 transition-all duration-700"
                                 :style="shown ? 'height: {{ $month['due'] > 0 ? min(100, ($month['due'] / max(1, collect($this->monthlyTrend)->max('due'))) * 100) : 0 }}%' : 'height: 0%'">
                            </div>
                            <!-- Foreground bar (completed) -->
                            <div class="w-3/4 rounded-t-lg absolute bottom-0 transition-all duration-1000 delay-300 {{ $month['rate'] >= 80 ? 'bg-green-500' : ($month['rate'] >= 60 ? 'bg-amber-500' : 'bg-red-500') }}"
                                 :style="shown ? 'height: {{ $month['due'] > 0 ? min(100, ($month['completed'] / max(1, collect($this->monthlyTrend)->max('due'))) * 100) : 0 }}%' : 'height: 0%'">
                            </div>
                        </div>
                        <span class="text-xs text-gray-500 font-medium">{{ Str::before($month['label'], ' ') }}</span>
                        <span class="text-xs font-bold {{ $month['rate'] >= 80 ? 'text-green-600' : ($month['rate'] >= 60 ? 'text-amber-600' : 'text-red-600') }}">{{ $month['rate'] }}%</span>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="relative" x-data="{ shown: false }" x-intersect="shown = true">
                    <svg class="w-40 h-40 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle cx="50" cy="50" r="45" fill="none" stroke="#d1d5db" stroke-width="10"
                                :stroke-dasharray="shown ? '{{ $gpDash }} {{ $circumference }}' : '0 {{ $circumference }}'"
                                stroke-dashoffset="0"
                                class="transition-all duration-1000"/>
                        <!-- ZapMed segment -->
                        <circle cx="50" cy="50" r="45" fill="none" stroke="#10b981" stroke-width="10"
                                :stroke-dasharray="shown ? '{{ $zapmedDash }} {{ $circumference }}' : '0 {{ $circumference }}'"
                                stroke-dashoffset="-{{ $gpDash }}"
                                class="transition-all duration-1000 delay-500"/>
                    </svg>
                    <div class="absolute inset-0 flex items-center justify-center">
                        <div class="text-center">
                            <p class="text-2xl font-black text-gray-900">{{ $this->renewalStats['renewed'] + $this->renewalStats['renewal_due'] }}</p>
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-bold text-gray-900">{{ $this->renewalStats['via_zapmed'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-bold text-gray-900">{{ $this->renewalStats['via_primary_doctor'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-bold text-gray-900">{{ $this->renewalStats['renewal_due'] }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-gradient-to-br from-green-50 to-green-100/50 rounded-xl p-4 text-center border border-green-100">
                    ...
                    <p class="text-3xl font-black text-green-700">{{ $this->orderStats['collections'] }}</p>
                    <p class="text-xs text-green-600 font-medium mt-1">Collections ({{ $this->orderStats['collection_pct'] }}%)</p>
                </div>
                <div class="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-4 text-center border border-blue-100">
                    ...
                    <p class="text-3xl font-black text-blue-700">{{ $this->orderStats['deliveries'] }}</p>
                    <p class="text-xs text-blue-600 font-medium mt-1">Deliveries ({{ 100 - $this->orderStats['collection_pct'] }}%)</p>
                </div>
            </div>
            <div class="space-y-3 border-t border-gray-100 pt-4">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-500">Total Orders</span>
                    <span class="text-sm font-bold text-gray-900">{{ $this->orderStats['total'] }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-500">Completed</span>
                    <span class="text-sm font-bold text-green-600">{{ $this->orderStats['completed'] }}</span>
                </div>
                @if($this->orderStats['avg_prep_minutes'])
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-500">Avg Preparation Time</span>
                        <span class="text-sm font-bold text-gray-900">{{ $this->orderStats['avg_prep_minutes'] }} min</span>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-3 gap-3 mb-6">
                <div class="text-center p-3 rounded-xl bg-gray-50">
                    <p class="text-3xl font-black text-gray-900">{{ number_format($this->patientStats['total']) }}</p>
                    <p class="text-xs text-gray-500 mt-1">Total</p>
                </div>
                <div class="text-center p-3 rounded-xl bg-green-50">
                    <p class="text-3xl font-black text-green-700">{{ number_format($this->patientStats['active']) }}</p>
                    <p class="text-xs text-green-600 mt-1">Active</p>
                </div>
                <div class="text-center p-3 rounded-xl bg-purple-50">
                    <p class="text-3xl font-black text-purple-700">+{{ $this->patientStats['new_this_period'] }}</p>
            ...
            <div class="mb-3">
                ...
                <div class="h-6 bg-gray-100 rounded-full overflow-hidden flex">
                    <div class="bg-green-500 h-full transition-all duration-700 flex items-center justify-center" style="width: {{ $optInW }}%">
                        @if($optInW > 10) <span class="text-[10px] font-bold text-white">{{ round($optInW) }}%</span> @endif
                    </div>
                    <div class="bg-gray-300 h-full transition-all duration-700 flex items-center justify-center" style="width: {{ $pendingW }}%">
                        @if($pendingW > 10) <span class="text-[10px] font-bold text-gray-600">{{ round($pendingW) }}%</span> @endif
                    </div>
                    <div class="bg-red-400 h-full transition-all duration-700 flex items-center justify-center" style="width: {{ $optOutW }}%">
                        @if($optOutW > 10) <span class="text-[10px] font-bold text-white">{{ round($optOutW) }}%</span> @endif
                    </div>
                </div>
                <div class="flex items-center gap-4 mt-2">
                    <div class="flex items-center gap-1"><div class="w-2 h-2 rounded-full bg-green-500"></div><span class="text-xs text-gray-500">Opted In ({{ $this->patientStats['consented'] }})</span></div>
                    ...
                    <div class="flex items-center gap-1"><div class="w-2 h-2 rounded-full bg-red-400"></div><span class="text-xs text-gray-500">Opted Out ({{ $this->patientStats['opted_out'] }})</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-1 px-3 py-1.5 bg-green-50 rounded-lg">
                ...
                <span class="text-xs font-medium text-green-700">{{ $this->pharmacyRanking->count() }} pharmacies</span>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($this->pharmacyRanking as $index => $pharmacy)
                        <tr class="hover:bg-green-50/30 transition">
                            <td class="p-4">
                                ...
                                @else
                                    <span class="text-gray-400 font-mono pl-2">{{ $index + 1 }}</span>
                                @endif
                            </td>
                            <td class="p-4">
                            ...
                            <td class="p-4 text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-xs font-bold">{{ $pharmacy->active_patients }}</span>
                            </td>
                            <td class="p-4 text-center">
                                <span class="text-sm font-medium text-gray-700">{{ $pharmacy->active_journeys }}</span>
                            </td>
                            <td class="p-4 text-center">
                                <span class="text-sm font-bold text-green-600">{{ $pharmacy->completed_orders }}</span>
                            </td>
                            <td class="p-4 text-center">
                                @php $perfScore = $pharmacy->active_patients > 0 ? min(100, round(($pharmacy->completed_orders / max(1, $pharmacy->active_patients)) * 100)) : 0; @endphp
                                <div class="flex items-center gap-2 justify-center">
                                    <div class="w-16 h-2 bg-gray-100 rounded-full overflow-hidden">
                                        <div class="h-full rounded-full {{ $perfScore >= 70 ? 'bg-green-500' : ($perfScore >= 40 ? 'bg-amber-500' : 'bg-red-400') }}" style="width: {{ $perfScore }}%"></div>
                                    </div>
                                    <span class="text-xs font-medium text-gray-500">{{ $perfScore }}%</span>
                                </div>
                            </td>
                        </tr>
                    @endforeach
