                ...
                <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width: 600px; width: 100%; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
                    ...
                    <tr>
                        <td style="padding: 40px;">
                            <h1 style="margin: 0 0 16px; font-size: 20px; font-weight: 600; color: #1f2937;">Your Prescription is Ready</h1>
                            <p style="margin: 0 0 24px; font-size: 15px; line-height: 1.6; color: #4b5563;">
                                Dr. <strong>{{ $prescription->doctor->last_name }}</strong> has signed your prescription. Here are the details:
                            </p>
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f9fafb; border-radius: 6px; margin-bottom: 24px;">
                                <tr>
                                    <td style="padding: 20px;">
                                        ...
                                        <p style="margin: 0 0 16px; font-size: 16px; font-weight: 600; color: #1f2937;">{{ $prescription->reference }}</p>
                                        <p style="margin: 0 0 8px; font-size: 13px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px;">Doctor</p>
                                        <p style="margin: 0 0 16px; font-size: 15px; color: #1f2937;">Dr. {{ $prescription->doctor->last_name }}</p>
                                        <p style="margin: 0 0 8px; font-size: 13px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px;">Medications</p>
                                        @foreach($prescription->items as $item)
                                        <p style="margin: 0 0 4px; font-size: 15px; color: #1f2937;">&#8226; {{ $item->medication_name }} {{ $item->strength }} ({{ $item->form }})</p>
                                        @endforeach
                                    </td>
                                </tr>
                            </table>
                            <p style="margin: 0 0 32px; font-size: 15px; line-height: 1.6; color: #4b5563;">
                                Your prescription has been signed and is ready. You can download it from your dashboard or it will be sent to our pharmacy partner for dispensing.
                            </p>
                            <a href="{{ url('/patient/prescriptions') }}" style="display: inline-block; padding: 12px 28px; background-color: #10b981; color: #ffffff; text-decoration: none; border-radius: 6px; font-size: 14px; font-weight: 600;">Download Prescription</a>
                        </td>
                    </tr>
                    <!-- Footer -->
                    <tr>
